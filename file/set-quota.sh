...
# set-quota.sh
#
# Menu interaktif untuk manage Xray bandwidth quota per-user.
# Daftar user diambil dari config.json (markers ### / #& / #! seperti
# bawaan add-vmess / add-vless / add-tr).
# ========================================================

DB="/usr/local/etc/xray/quota-xray.db"
CFG="/usr/local/etc/xray/config.json"
QUOTA_BIN="/usr/local/bin/quota-xray"
...
# Collect unique users from xray config.json markers + DB
collect_users() {
  {
    grep -E '^### [A-Za-z0-9_]+ ' "$CFG" 2>/dev/null | awk '{print $2}'
    grep -E '^#& [A-Za-z0-9_]+ '  "$CFG" 2>/dev/null | awk '{print $2}'
    grep -E '^#! [A-Za-z0-9_]+ '  "$CFG" 2>/dev/null | awk '{print $2}'
...
pick_user() {
  ...
  if [ "${#USERS[@]}" -eq 0 ]; then
    echo -e "${YLW}Belum ada user Xray.${NC}"
    return 1
  fi
  echo -e "${CYN}─────────────────────────────────────────────${NC}"
...
main_menu() {
  while :; do
    clear
    echo -e "${CYN}─────────────────────────────────────────────${NC}"
    echo -e "${CYN}        Xray Bandwidth Quota Manager${NC}"
    echo -e "${CYN}─────────────────────────────────────────────${NC}"
    echo
    ...
    echo "  5) Lihat status semua user (cek-quota)"
    echo "  0) Keluar"
    echo
    read -rp "Pilihan: " opt
    case "$opt" in
      ...
      3) menu_block ;;
      4) menu_unblock ;;
      5) /usr/local/sbin/cek-quota 2>/dev/null || cek-quota; pause ;;
      0) exit 0 ;;
      *) ;;
    esac
  done
