...
# cek-quota.sh
#
# Display per-user Xray bandwidth usage + quota + status, sorted by usage.
# DB-nya di-maintain oleh /usr/local/bin/quota-xray (cron tiap 1 menit).
# ========================================================
...
if [ ! -f "$DB" ] || [ ! -s "$DB" ]; then
  echo "────────────────────────────────────────"
  echo "  Xray Quota: belum ada user ter-tracking."
  echo "  Pastikan quota-xray cron sudah jalan + ada user xray yang aktif."
  echo "────────────────────────────────────────"
  exit 0
fi
